...
Die wählbaren Invoice-Modi gehören zu 2.1.0. Der aktuelle Arbeitsstand ist `2.1.0-rc.11` und nur für kontrollierte Test- und Nachlaufinstallationen vorgesehen. Der Voucher-, Booking- und Korrekturumfang aus 2.0.0 bleibt erhalten. RC.11 übernimmt den Direktbetrieb und die Late-Fee-Trennung aus rc.9 sowie den eng begrenzten 0-%-Altbestandsnotweg aus rc.10. Neu hinzugekommen ist die bereits per Canary abgesicherte Rule-17-Invoice für historische Drittlandrechnungen mit genau einem `PromoHosting`-Rabatt. Alle neuen Abläufe sind standardmäßig ausgeschaltet.
...
Wenn sevDesk Rule 11 nach einem Wechsel der Besteuerungsart nicht mehr fertigstellen kann, gibt es im Sammelexport einen separaten, nicht automatisch aktiven Notweg. Er ist kein Ersatz für die richtige Steuerregel. Nach ausdrücklicher Bestätigung erzeugt er für bezahlte EUR-Rechnungen aus dem fest begrenzten Kleinunternehmerzeitraum mailfreie Rule-17-Belege. Positive Rechnungen werden als Voucher auf einem aktuell von `ReceiptGuidance` bestätigten 0-%-Erlöskonto angelegt. Drittlandrechnungen mit genau einem strukturell belegten `PromoHosting`-Rabatt dürfen als Invoice folgen, wenn der exakte Rule-17-Rabatt-Canary bereits bestanden ist; für Invoices kann kein eigenes `accountDatev` gewählt werden. Guthaben, Sammelzahlungen, AddFunds, Late Fees und andere Rabatt- oder Steuerfälle bleiben draußen. Alle so erfassten Belege müssen vor jeder sevDesk-Auswertung oder Abgabe manuell richtig zugeordnet werden.
...
| [RELEASE_NOTES_2.1.0-rc.11.md](RELEASE_NOTES_2.1.0-rc.11.md) | Release Notes für die aktuelle Vorabversion |
