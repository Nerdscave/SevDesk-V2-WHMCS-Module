...
- Release 2.1.0-rc.9 ergänzt zwei eingefrorene Invoice-Lebenszyklen. `after_payment_proforma` bleibt der Upgrade-Default. `issue_on_creation` ist nur mit `invoice_only`, sevDesk-Hoheit, abgeschalteter WHMCS-Proforma, abgeschalteter fortlaufender Paid-Nummerierung, unverändertem Rechnungsdatum, Theme-Adapter-Vertrag v2 und bestandenem Direktbetriebs-Canary zulässig. Bestehende Mappings wechseln ihren Lebenszyklus nicht rückwirkend.
- Im Direktbetrieb bleibt WHMCS Quelle für Forderung, Fälligkeit, Mahnstufe und Zahlung. WHMCS plant die Termine, das Modul unterdrückt die parallelen Core-Mails und sevDesk liefert die kundenseitig sichtbaren `RE`-, `MA`- und `SR`-Dokumente. Es gibt keinen stillen Rückfall auf WHMCS-PDF oder WHMCS-Mail.
- Positive, unbesteuerte `LateFee`-Positionen dürfen ausschließlich über `late_fee_mode=reminder_then_rule22` aus der Leistungs-Invoice herausgelöst werden. Die Gebühr wird entweder nach nachgewiesener buchhalterischer Wirkung genau einmal durch die sevDesk-Mahnung oder nach Zahlung durch einen verknüpften Rule-22-Revenue-Voucher mit aktuell bestätigtem `REVENUE`-Konto erfasst. Beide Quellen gleichzeitig sind verboten. Andere, negative, besteuerte oder nachträglich veränderte Gebühren bleiben blockiert.
- Direktbetrieb, Mahnversand, Invoice-Storno, ZUGFeRD-Storno und Rule-22-Gebührenpfad besitzen getrennte, standardmäßig ausgeschaltete Canaries. Teilzahlungen blockieren automatische Mahnungen. Ein bereits ausgegebener und danach materiell veränderter Beleg wird eingefroren und als Prüffall behandelt.
...
- Die Kleinunternehmerregelung kann mit einem Enddatum begrenzt werden. Bis einschließlich dieses Rechnungsdatums gilt Rule 11 mit 0 % vor jeder Länder-, Kunden- oder AddFunds-Klassifikation. Ein leeres Enddatum behält aus Kompatibilitätsgründen die bisherige unbegrenzte Regel bei.
- Rule-11-Invoices bleiben hinter einem eigenen, standardmäßig ausgeschalteten Canary. Zusätzlich muss die aktuelle `ReceiptGuidance` mindestens ein numerisches `REVENUE`-Konto ausweisen, das Rule 11 mit 0 % erlaubt. Fehlt einer dieser Nachweise, endet der Vorgang vor dem ersten Beleg-Write. Rule-11-Voucher verwenden unverändert ihr geprüftes `accountDatev`.
- `2.1.0-rc.11` enthält einen ausdrücklich manuellen Notweg für historische 0-%-Kleinunternehmerbelege, die der aktuelle sevDesk-Mandant nicht mehr mit Rule 11 fertigstellen kann. Er ist nur im bestätigten Altbestandsformular erreichbar und versendet keine Mail. Positive EUR-Rechnungen werden mit Rule 17 und einem frisch durch `ReceiptGuidance` bestätigten `REVENUE`-Konto als Voucher angelegt. Genau ein strukturell belegter `PromoHosting`-Rabatt darf nur bei Drittlandskunden, `invoice_only` und passendem Rule-17-Rabatt-Canary als Invoice übertragen werden; dabei ist kein eigenes `accountDatev` möglich. Guthaben, Sammelzahlungen, andere Rabatte, AddFunds, Late Fees, Fremdwährungen und Belege außerhalb des festen Kleinunternehmerzeitraums bleiben gesperrt. Die Kontierung ist ausdrücklich vorläufig und muss vor jeder sevDesk-Auswertung oder Abgabe manuell berichtigt werden; der normale Tax-Resolver bleibt unverändert.
- Reine WHMCS-Sammelzahlungsrechnungen sind Zahlungscontainer und keine Umsatzbelege. Nur eine vollständig bewiesene Kette darf die Originalrechnungen freigeben. Bei ihnen gilt centgenau `subtotal + tax + tax2 = total + credit`: `total` ist der direkte Zahlteil, `total + credit` der Dokumentbrutto. Positive `tblaccounts`-Zahlungen müssen `total` entsprechen. Die gemeinsame Banktransaktion bleibt manuell; unvollständige oder veränderte Ketten blockieren vor dem Beleg-Write.
- Genau ein über Typ, `relid` und Steuerkennzeichen eindeutig zugeordneter `PromoHosting`-Rabatt darf ausschließlich in `invoice_only`, in EUR und außerhalb des E-Rechnungspfads als negative `InvoicePos` übertragen werden. Der globale sevDesk-Rabatt `discountSave` bleibt dabei leer, weil er bei steuerpflichtigen Bruttorechnungen die WHMCS-Centverteilung zwischen Netto und Steuer verändern kann. Freigegeben sind nur Rule 11 mit 0 %, Rule 1 mit 19 %, Rule 17 mit 0 % sowie Rule 19 mit positivem, einheitlichem Zielsteuersatz. Der deutsche Rule-1-Fall und EU-B2C mit bestätigter Inlandsbesteuerung haben getrennte Gates und Capability-Keys. Die standardmäßig ausgeschalteten Gates für Rules 1/17/19 speichern den exakten Schlüssel aus Profil, Länderklasse, Rate und WHMCS-Netto-/Bruttomodus. Bei Rule 11 bleiben der allgemeine Rule-11-Invoice-Canary und die aktuelle `REVENUE`-Guidance zusätzlich Pflicht. Capability-Key und Rabatt-Fingerprint werden vor dem Write eingefroren. Readback und Recovery verlangen die exakten Remote-Positionen und `sumNet`-, `sumTax`- und `sumGross`-Werte; `sumDiscounts` muss numerisch 0 sein. Andere negative Positionen und E-Rechnungen mit Rabatt bleiben blockiert. `LateFee` ist keine negative Invoice-Position und folgt ausschließlich dem getrennten rc.9-Gebührenvertrag.
...
- Automatische Refund-/Chargeback-Ausführung oder Massenerzeugung, allgemeine Gutschriften und `CreditNote`-Fallbacks bleiben ausgeschlossen. Die einzige automatische Storno-Ausnahme ist eine unbezahlte, unverändert ausgegebene Direktbetriebs-Invoice hinter dem bestätigten `cancelInvoice`-Canary. Andere Stornos bleiben Prüffälle. Die UI darf Rückzahlungen zur manuellen Auswahl anzeigen; Chargebacks werden blockiert.
...
- Der normale Invoice-Export blockiert negative Positionen weiterhin. Die einzige Ausnahme ist die eng freigegebene negative `PromoHosting`-`InvoicePos` aus den festen Entscheidungen; `discountSave` darf diesen Sonderfall nicht ersetzen. Weitere negative Remote-Positionen dürfen nur im ausdrücklich bestätigten `CorrectionService`-Pfad entstehen. Dort werden positive Eingabepositionen erst nach der Tax-/Guidance-Prüfung in negative Voucher-Positionen übersetzt.
...
- `create_invoice_reminder`, `cancel_invoice` und `export_late_fee_voucher` besitzen eigene Requested-, Created-, Verified-, Mapping- und Delivery-Checkpoints. Nach einem möglicherweise ausgeführten Create-, Cancel- oder Mail-Write darf nur noch gelesen werden. Das additive `mod_sevdesk_related_documents` speichert die Rollen `reminder`, `cancellation` und `late_fee_voucher`; es enthält keine Adress-, Mail-, PDF- oder API-Rohdaten.
