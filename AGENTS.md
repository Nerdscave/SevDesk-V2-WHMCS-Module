...
- Genau ein über Typ, `relid` und Steuerkennzeichen eindeutig zugeordneter `PromoHosting`-Rabatt darf ausschließlich in `invoice_only` mit Rule 11, 0 %, EUR und bestandenem Rabatt-Canary als `discountSave` übertragen werden. Andere negative Positionen bleiben blockiert.
